pipeline{
    ...
    stages{
        stage('Test Build'){
            steps{
                script{
                    ...
                    archive (includes: 's1_html/index.html', 's2_html/index.html', 's3_html/index.html', 's4_html/index.html')
                }
            }
        }
        stage('Build Images'){
        ...
        stage('Deploy app'){
            steps{
                sh 'ssh -o StrictHostKeyChecking=no jenkins@swarm-manager "export duri=${DATABASE_URI}"'
