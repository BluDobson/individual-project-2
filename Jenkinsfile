pipeline{
    ...
    stages{
        ...
        stage('Deploy app'){
            steps{
                sh 'ssh -o StrictHostKeyChecking=no jenkins@swarm-manager "export app_version=${app_version} DATABASE_URI=${DATABASE_URI}"'
                sh 'ssh -o StrictHostKeyChecking=no jenkins@swarm-manager "echo $app_version"'
